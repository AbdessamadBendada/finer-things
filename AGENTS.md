...
The active implementation deliverable is the standalone Next.js site in the sibling `../alex-next/`. The connected static site in `finer-things-website/` remains preserved, and the twelve files in `finer-things-website/luxury-motion-studies/` are the authoritative evolving visual/content references for the Next.js transcription. When an experiment is approved, update the pinned Next.js fixtures deliberately rather than assuming they remain byte-identical.
